        ...
        <div class="row">
            ...
            <div class="col-xs-6 col-sm-6">
                <ul class="top-link">
                    ...
                    <li><a href="${pageContext.request.contextPath}/orders.do?uid=${sessionScope.user.uid}" class="btn btn-info"><span
                    ...
                    <c:otherwise>
                        <div class="table-responsive">
                            <table class="table table-striped table-hover table-condensed table-bordered col-lg-12" align="center" >
                                ...
                                <tbody>
                                <c:forEach items="${requestScope.pageBean.list}" var="c">
                                    <tr>
                                        <td>${c.cid}</td>
                                        <td><a href="${pageContext.request.contextPath}/product.do?pid=${c.productBean.pid}">${c.productBean.productName}</a></td>
                                        <td>¥${myel:priceTwoDecimal(c.productBean.price)}</td>
                                        <td><a href="javascript:void(0)" onclick="nowCheck(${c.cid})">取消收藏</a></td>
                                    </tr>
                                </c:forEach>
                                </tbody>
                            </table>
                ...
                <ul class="pagination">
                    <c:choose>
                        <c:when test="${requestScope.pageBean.currentPage != requestScope.pageBean.previousPage}">
                            <li><a href="${pageContext.request.contextPath }/collection.do?currentPage=${requestScope.pageBean.previousPage}" >上一页</a></li>
                        </c:when>
                        <c:otherwise>
                    ...
                    <c:forEach var="pagenum" items="${requestScope.pageBean.pageBar }">
                        <c:choose>
                            ...
                            <c:otherwise>
                                <li><a href="${pageContext.request.contextPath }/collection.do?currentPage=${pagenum}">${pagenum }</a></li>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                    <c:choose>
                        <c:when test="${requestScope.pageBean.currentPage != requestScope.pageBean.nextPage}">
                            <li><a href="${pageContext.request.contextPath }/collection.do?currentPage=${pageBean.nextPage}" >下一页</a></li>
                        </c:when>
                        <c:otherwise>
...
<script>
    var nowCheck = function (cid) {
        if(window.confirm("亲,确认取消收藏吗?")){
            window.location.href = "${pageContext.request.contextPath}/removeCollection.do?cid=" + cid;
        }
    }
</script>
</body>
